...
import { getClonesRedis, normalizeCloneId, unicCloneKey } from '@/lib/clones';
  ...
  try {
    ...
    if (shouldIncrement) {
      count = await redis.incr(key).catch((err) => {
        console.error('Unic clones counter incr error:', err);
        return 0;
      });
    } else {
      count =
        (await redis.get<number>(key).catch((err) => {
          console.error('Unic clones counter get error:', err);
          return null;
        })) ?? 0;
    }
    ...
    console.error('Unic clones counter error:', error);
